...
async function create (req, res) {
  ...
  const { project } = req.context
  const { department, date, entries = [], notes = '' } = handleFormValues(req.body)
  const reportDepartment = await getDepartment(department)

  const timeRecords = await createTimeRecords({ date, department, entries, project: project.id })
  const recordNote = await createRecordNote({ date, department, notes, project: project.id })
